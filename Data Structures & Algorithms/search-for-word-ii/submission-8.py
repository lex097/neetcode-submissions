    ...
    def findWords(self, board: List[List[str]], words: List[str]) -> List[str]:
        ...
        for i in board:
            ...
            for k in i:
                node = TrieNode(k)
                row.append(node)
                self.nodeMap[k].append(node)
            self.nodeBoard.append(row)
        for i in range(0, len(self.nodeBoard)):
            for k in range(0, len(self.nodeBoard[i])):
                #i k+1, i k-1, i+1 k, i-1 k
                if k != (len(self.nodeBoard[i]) - 1):
                    self.nodeBoard[i][k].childMap[self.nodeBoard[i][k+1].char].append(self.nodeBoard[i][k+1])
                if k != 0:
                    self.nodeBoard[i][k].childMap[self.nodeBoard[i][k-1].char].append(self.nodeBoard[i][k-1])
                if i != (len(self.nodeBoard) - 1):
                    self.nodeBoard[i][k].childMap[self.nodeBoard[i+1][k].char].append(self.nodeBoard[i+1][k])
                if i != 0:
                    self.nodeBoard[i][k].childMap[self.nodeBoard[i-1][k].char].append(self.nodeBoard[i-1][k])

        def dfs(node, word, idx, used):
            if idx == len(word):
                self.inTrie = True
                return
            
            if word[idx] in node.childMap:
                for child in node.childMap[word[idx]]:
                    if child not in used:
                        used.add(child)
                        dfs(child, word, idx + 1, used)
                        used.remove(child)
                        if self.inTrie:
                            return
            return
        for i in words:
            self.inTrie = False
            if i[0] in self.nodeMap:
                for k in self.nodeMap[i[0]]:
                    used = {k}
                    dfs(k, i, 1, used)
                    if self.inTrie:
                        break
